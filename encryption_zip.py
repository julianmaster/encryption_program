...
class ZipUtilities:
    ...
    def add_file(self, source_file, destination_file=None, run_substitution = True):
        ...
        if source_ext == ".py":
            ...
            with open(os.path.join("build", source_basename), "w") as temp_file:
                temp_file.write(temp_text)
        else:
            shutil.copy(source_file, os.path.join("build", source_basename))

        # Compile and zip file
        if source_ext == ".py":
            py_compile.compile(os.path.join("build", source_basename), os.path.join("build", destination_file))
        self.zip_file.write(os.path.join("build", destination_file), arcname=destination_file)

        # Remove files
        os.remove(os.path.join("build", source_basename))
        os.remove(os.path.join("build", destination_file))
